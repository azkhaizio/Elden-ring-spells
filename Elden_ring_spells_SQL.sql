...
select DMG_Type, COUNT(DMG_Type) as Number_spell, avg(Net_AR) as avg_dam
from final_table
group by DMG_Type
...
select spell_attribute,count(spell_attribute) as number
from spell_att
...
select type, avg(Net_AR) as dam_spell
from final_table
where Net_AR != 0
group by type
